...
draw2d.Workflow=function(/*:String*/id)
{
   try
   {
      ...
      // Ein Workflow enthÃ¤lt Elemente
      //
      /** @private */
...
draw2d.Workflow.prototype.getToolWindow=function()
{
...
{    
    ...
    if(figure instanceof draw2d.Connection) {        
        //remove a cardinalidade do elemento target
        portTarget = figure.getTarget();			
	if (document.getElementById(portTarget.getId()) != null) {
		idPortPai = document.getElementById(portTarget.getId()).parentNode.id;
		this.getFigure(idPortPai).removeCardinal(idPortPai, true, this, true);
	}			
        
        figure.disconnect();

        figs = this.getFigures();
        for(var i=0;i<figs.getSize();i++) {
            ports = document.getElementById(figs.get(i).getId()).getElementsByTagName('div');
            for (j=0;j<ports.length;j++) {
                //impede que a linha do operador/precondicao fique com mouse = pointer, o certo é tentar setar os ids das Ports
                if (!ports[j].id.match("_operador") && !ports[j].id.match("_precondicao") && !ports[j].id.match("_card") && !ports[j].id.match("_exec"))
                    ports[j].style.cursor = "pointer";
            }
        }
    }
    ...
    if(figure instanceof draw2d.Figure) {
        if (figure.getSubType() == "PreCondicaoL" ||
        figure.getSubType() == "PreCondicaoR" ||
        figure.getSubType() == "Meta" ||
        figure.getSubType() == "Tarefa" ||
        figure.getSubType() == "TarefaOpcional" ||
        figure.getSubType() == "Operador") 
        {
            //remove cardinalidades
            figure.removeCardinal(figure.getId(), true, this);

            //remove input connections
            connections = figure.getInConnections();
            if (connections != null) {
                for(var i=0;i<connections.getSize();i++)
                {
                    con = connections.get(i);
                    this.removeFigure(con);
                }
            }
            
            //remove output connections
            connections = figure.getOutConnections();
            if (connections != null) {
                for(var i=0;i<connections.getSize();i++)
                {
                    con = connections.get(i);
...
draw2d.Workflow.prototype.getFig=function(ii)
{
     var figure = this.figures.get(ii);
     return figure;
...
/**
 * validacao do diagrama
 **/
draw2d.Workflow.prototype.valida=function()
{
    var conteudo = "";
    var totalMeta = 0;
    var elementos = 0;
    var precondicaoerro = false;
    var metaerro = false;    
    var ciclo = 0;
    var semTexto = 0;
    
    var erromuitospais = this.erroMuitosPais();
    
    for(var i=1; i<this.figures.getSize();i++)
    {
        figure = this.figures.get(i);
        if(figure instanceof draw2d.Figure) {
            if (figure.getSubType() == "Meta")
                totalMeta++;
			
            if (figure.getSubType() != "caixatexto")
                elementos += this.nroConexoes(figure);
            
            if (figure.getSubType() == "Meta" && !metaerro)
                metaerro = this.erroMeta(figure);

            ciclo += this.ciclo(figure, figure.getId());
            
            semTexto += this.hasTexto(figure);
        }
    }

    if (erromuitospais) {
        conteudo += "<a href='javascript:void(0)' alt='Erro' title='Erro' onClick='openWindow(\"ajuda.html\")'><img alt='Erro' title='Erro' src='images/error.png'/> Uma tarefa n&atilde;o pode ter mais de um pai.<br/><br/></a>";
    }
    if (elementos > 0) {
        conteudo += "<a href='javascript:void(0)' alt='Erro' title='Erro' onClick='openWindow(\"ajuda.html\")'><img alt='Erro' title='Erro' src='images/error.png'/> O seu diagrama de tarefas possui "+elementos+" elemento(s) sem conex&otilde;es.<br/><br/></a>";
    }
    if (ciclo > 0) {
        conteudo += "<a href='javascript:void(0)' alt='Erro' title='Erro' onClick='openWindow(\"ajuda.html\")'><img alt='Erro' title='Erro' src='images/error.png'/> O seu diagrama de tarefas possui um ou mais ciclos.<br/><br/></a>";
    }
    if (semTexto > 0) {
        conteudo += "<a href='javascript:void(0)' alt='Erro' title='Erro' onClick='openWindow(\"ajuda.html\")'><img alt='Erro' title='Erro' src='images/error.png'/> O seu diagrama de tarefas possui "+semTexto+" elemento(s) sem texto.<br/><br/></a>";
    }
    if (totalMeta == 0) {
        conteudo += "<a href='javascript:void(0)' alt='Erro' title='Erro' onClick='openWindow(\"ajuda.html\")'><img alt='Erro' title='Erro' src='images/error.png'/> O seu diagrama de tarefas deve possuir pelo menos 1 (uma) Meta.<br/><br/></a>";
    }
    else if (totalMeta > 1) {
        conteudo += "<a href='javascript:void(0)' alt='Alerta' title='Alerta' onClick='openWindow(\"ajuda.html\")'><img alt='Alerta' title='Alerta' src='images/alert.png'/> O seu diagrama possui mais de uma meta.<br/><br/></a>";
    }
    if (metaerro) {
        conteudo += "<a href='javascript:void(0)' alt='Alerta' title='Alerta' onClick='openWindow(\"ajuda.html\")'><img alt='Alerta' title='Alerta' src='images/alert.png'/> Uma Meta deve ser pai de tarefa(s).<br/><br/></a>";
    }
    if (totalMeta == 1 && elementos == 0 && !metaerro && !erromuitospais && !ciclo && semTexto == 0) {
        conteudo = "<a href='javascript:void(0)' alt='OK' title='OK' onClick='openWindow(\"ajuda.html\")'><img alt='OK' title='OK' src='images/validate.png' height='12' width='12'/> Nenhum erro ou alerta encontrado.<br/><br/></a>";
    }
    conteudo += "<br/><a href='javascript:void(0)' alt='Dica' title='Dica' onClick='openWindow(\"ajuda.html\")'><img alt='Dica' title='Dica' src='images/lamp.png' height='12' width='12'/> Na meta, lembre-se de utilizar a mesma letra informada no Diagrama Hier&aacute;rquico de Metas, caso esse tenha sido constru&iacute;do.<br/><br/></a>";
    	
    document.getElementById('workflowValidaResult').innerHTML = conteudo;
};


draw2d.Workflow.prototype.nroConexoes=function(/*:Figure*/ figure)
{
    connectIn = 0;
    connectOut = 0;
    inputPorts = figure.getInputPorts();
    for(var j=0;j<inputPorts.getSize();j++)
    {
        connections = inputPorts.get(j).getConnections();
        for(var k=0;k<connections.getSize();k++)
        {
            portSource = connections.get(k).getSource();
            idPortPai = document.getElementById(portSource.getId()).parentNode.id;
            figureChild = this.getFigure(idPortPai);
            if (!figureChild.getSubType().match("PreCondicao"))
                connectIn++;
        }
    }
    outputPorts = figure.getOutputPorts();
    for(var j=0;j<outputPorts.getSize();j++)
    {
        connections = outputPorts.get(j).getConnections();
...
draw2d.Workflow.prototype.erroPreCondicao=function(/*:Figure*/ figure)
{
    outputPorts = figure.getOutputPorts();
    for(var i=0;i<outputPorts.getSize();i++)
    {
        ...
        for(var j=0;j<connections.getSize();j++)
        {
            ...
            idPortPai = document.getElementById(portTarget.getId()).parentNode.id;
            figureChild = this.getFigure(idPortPai);
            if (figureChild.getSubType() == "Meta")
                return false;
            else
                return true;
        }
...
draw2d.Workflow.prototype.erroMeta=function(/*:Figure*/ figure)
{
    inputPorts = figure.getInputPorts();
    for(var i=0;i<inputPorts.getSize();i++)
    {
        ...
        for(var j=0;j<connections.getSize();j++)
        {
            portSource = connections.get(j).getSource();
            idPortPai = document.getElementById(portSource.getId()).parentNode.id;
            figureParent = this.getFigure(idPortPai);
            if (figureParent.getSubType().match("PreCondicao"))
                return false;
            else
                return true;
        }
...
draw2d.Workflow.prototype.erroMuitosPais=function()
{
    targets = new draw2d.ArrayList();
    for(var h=1; h<this.figures.getSize();h++)
    {
        figure = this.figures.get(h);
        if(figure instanceof draw2d.Figure) {
            outputPorts = figure.getOutputPorts();
            for(var i=0;i<outputPorts.getSize();i++)
            {
                ...
                {
                    portTarget = connections.get(j).getTarget();
                    idPortTarget = document.getElementById(portTarget.getId()).parentNode.id;
                    
                    portSource = connections.get(j).getSource();
                    idDivPortSource = document.getElementById(portSource.getId()).parentNode.id;
                    figureSource = this.getFigure(idDivPortSource);
                    
                    if (!figureSource.getSubType().match("PreCondicao")) {
                        if (targets.contains(idPortTarget))
                            return true;
                        else
                            targets.add(idPortTarget);
                    }
                }
            }
        }
...
draw2d.Workflow.prototype.ciclo=function(/*:Figure*/ figure, /*:String*/idFigure)
{
    outputPorts = figure.getOutputPorts();
    for(var i=0;i<outputPorts.getSize();i++)
    {
        ...
        for(var j=0;j<connections.getSize();j++)
        {
            portTarget = connections.get(j).getTarget();
            idPortTarget = document.getElementById(portTarget.getId()).parentNode.id;
            figureParent = this.getFigure(idPortTarget);            
            if (idPortTarget == idFigure) {
                return 1;
            } else {
                return 0 + this.ciclo(figureParent, idFigure);
            }
        }
    }
    return 0;
...
draw2d.Workflow.prototype.hasTexto=function(/*:Figure*/ figure)
{
    texto = figure.getTextIn();
    if (texto != null)
        return 0;
    else
        return 1;
...
/**
 * @returns lista com os objetos da workflow
 * comeca em 1 pos a tools e a 1a posicao.
 * se mudar tem q comecar em 0 diogo 14/05
 * posição,subtype, altura, largura, posicÃ£o x, posicao y,texto,numero de filhos,id dos filhos |
 * separacao dos itens feitas por / e o fim eh marcado por um |
 **/
draw2d.Workflow.prototype.getListaFigure=function()
{
  var retorno = '';
  for(var i=1; i<this.figures.getSize();i++)
  { 
     retorno += i + '/';
     retorno += this.figures.get(i).getSubType() + '/'; 
     retorno += this.figures.get(i).getHeight() + '/';
     retorno += this.figures.get(i).getWidth() + '/';
     retorno += this.figures.get(i).getAbsoluteX() + '/';
     retorno += this.figures.get(i).getAbsoluteY() + '/';
     retorno += this.figures.get(i).getTextIn() + '/'; 
     retorno += this.figures.get(i).getCard() + '/';
     retorno += this.figures.get(i).getExec() + '/';
     var IDpai = this.figures.get(i).getPai();
     if(IDpai != null){
        for(var x=1; x<this.figures.getSize();x++){
            if(this.figures.get(x).getId() == IDpai){
                retorno += x;
            }
        }
     }
...
/**
 * altera o id da figura recebendo a posicção dela e o novo id.
 * FUNÇÃO ABRIR
 **/

draw2d.Workflow.prototype.alteraID=function(posicao,id)
{
    this.figures.get(posicao).setId(id)

};

/**
 *  altera a dimensão do objeto na posição informada.
 *  FUNÇÃO ABRIR
 **/

draw2d.Workflow.prototype.setDimensoes=function(posicao,w,h)
{
    //posicao = posicao.trim();
    if (posicao != null){
        this.figures.get(posicao).setDimension(w,h);}
    this.showResizeHandles(this.figures.get(posicao));

};

/**
 * cria ligação entre dois objetos recebe a posição de cada um p1 = pai, p2 = filho
 * FUNÇÃO ABRIR
 **/
draw2d.Workflow.prototype.liga=function(p1,p2,workflow)
{
    var port1 = this.figures.get(p1).getPortaSai();
    var port2 = this.figures.get(p2).getPortaEnt();
    var con = new draw2d.CommandConnect(workflow, port1, port2);
    con.execute();
};
 ...
 * FUNÇÃO ABRIR
 **/
draw2d.Workflow.prototype.setTextoo=function(posicao,texto)
{
		var id = this.figures.get(posicao).getId()+"_text";
		var idIn = id+"_in";    
		var idFigure = this.figures.get(posicao).getId();
		var id = idFigure + "_text";
		var idIn = id+"_in";
		var dialogText = texto;
		var str = texto;
                var figure = this.figures.get(posicao);
    ...
    var te = "";
    this.figures.get(posicao).getWorkflow().getCommandStack().execute(new draw2d.CommandSetText(this.figures.get(posicao), str, texto, hasPreCond, side));
};

/**
 * Adiciona Cardinalidade
 * FUNÇÃO ABRIR
 **/
draw2d.Workflow.prototype.setCardinalidade=function(posicao,card)
{
    this.figures.get(posicao).setCard(card);
};

draw2d.Workflow.prototype.setCardinalidadeFromPai=function(posicao,card)
{
    this.figures.get(posicao).addCardinal(card);
};

/**
 * Adiciona Executabilidade?!?!?!
 * FUNÇÃO ABRIR
 **/
draw2d.Workflow.prototype.setExe=function(posicao,exec)
{
    this.figures.get(posicao).setExec(exec);
};

draw2d.Workflow.prototype.setExeFromPai=function(posicao,card)
{
    this.figures.get(posicao).addExecucaoDiv(card);
};

draw2d.Workflow.prototype.ativaFigures=function()
{
    for(var i=1; i<this.figures.getSize();i++)
    {
        figure = this.figures.get(i);
